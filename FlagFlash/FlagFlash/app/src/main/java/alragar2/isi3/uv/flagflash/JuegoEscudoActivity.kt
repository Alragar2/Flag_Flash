...
class JuegoEscudoActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buttons = listOf(
            findViewById<Button>(R.id.Escudo1),
            findViewById<Button>(R.id.Escudo2),
            findViewById<Button>(R.id.Escudo3),
            findViewById<Button>(R.id.Escudo4)
        )
        ...
        databaseReference.child("escudos").get().addOnSuccessListener { dataSnapshot ->
            if (dataSnapshot.exists()) {
                for (countrySnapshot in dataSnapshot.children) {
                    val escudoUrl = countrySnapshot.child("escudo").value.toString()
                    escudoUrl?.let { countryShiedsURL.add(it) }
                }

                CoroutineScope(Dispatchers.IO).launch {
                    for (url in countryShiedsURL) {
                        try {
                            val futureTarget = Glide.with(this@JuegoEscudoActivity)
                                .load(url)
                                .submit()
                            futureTarget.get()
                        } catch (e: Exception) {
                            Log.e("JuegoEscudoActivity", "Error al cargar imagen: $url", e)
                        }
                    }
                }
            }
        }.addOnFailureListener() { e ->
            Log.e("JuegoEscudoActivity", "Error al obtener datos", e)
        }
    }
